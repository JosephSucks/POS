...
export default function ReserveTableModal({ isOpen, onClose, tableId, tableName }: ReserveTableModalProps) {
  ...
  // Initialize times with current time + 2 hours
  useEffect(() => {
    if (isOpen && reservationType === "timed") {
      const now = new Date()
      const from = new Date(now.getTime() + 30 * 60000) // 30 minutes from now
      const to = new Date(from.getTime() + 120 * 60000) // 2 hours later
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (reservationType === "timed") {
      ...
      // Check if from time is in the past
      if (from < now) {
    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side="bottom" className="h-auto max-h-[90vh] rounded-t-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Time Selection (for timed reservations) */}
          {reservationType === "timed" && (
