...
export default function ReserveTableModal({ isOpen, onClose, tableId, tableName }: ReserveTableModalProps) {
  ...
  const parseDateTime = (dateStr: string, timeStr: string) => {
    // Create date in local timezone by parsing YYYY-MM-DD and HH:MM
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (reservationType === "timed") {
      ...
      const reservationDetails: ReservationDetails = {
        ...
        // Also send local time strings (without Z suffix) for storage
        fromLocalString: toLocalISOString(from),
        toLocalString: toLocalISOString(to),
      }
  ...
  const toLocalISOString = (date: Date) => {
    // Convert to local ISO string format (YYYY-MM-DDTHH:mm:ss) without Z suffix
