...
export default function ReservationDetailsModal({ isOpen, onClose, table }: ReservationDetailsModalProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const now = new Date()
      // Parse local time string (without Z suffix means it's local time)
  ...
  const formatDateTime = (dateStr?: string) => {
    if (!dateStr) return "-"
    // Parse local time string (YYYY-MM-DDTHH:mm:ss without Z)
    const date = new Date(dateStr)
    // Format in user's local timezone using 12-hour format
  ...
  const formatFullDateTime = (dateStr?: string) => {
    ...
    return `${dateStr2} ${timeStr} ${tzName}`
  }
