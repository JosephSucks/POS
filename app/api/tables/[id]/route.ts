  ...
  try {
    ...
    if (action === "reserve") {
      ...
      if (type === "timed") {
        ...
        // Parse local time strings (without Z suffix, so they're treated as local)
      ...
      // Store local time strings as-is (without UTC conversion)
      const result = await sql`
        UPDATE "tables"
        SET 
          status = 'reserved',
          ...
          reserved_from = ${type === "timed" ? from : null},
          reserved_to = ${type === "timed" ? to : null},
      ...
      const overlapping = await sql`
        ...
        AND status = 'reserved'
        AND reserved_from < ${to}
        AND reserved_to > ${current[0].reserved_from}
      `
      ...
      // Extend reservation - store local time string as-is
      const result = await sql`
        UPDATE "tables"
        SET 
          reserved_to = ${to},
